...
def create_updating(request):
    ...
    updating_type = body_json.get("updating_type")
    ...
    if not version or not updating_type or not deploy_type:
        return HttpResponse(json.dumps({"state": 0, "message": "Dữ liệu không hợp lệ"}))

    is_exists_version = Updating.objects.filter(project_version=version)
    ...
    new_update = Updating(  project=project,
